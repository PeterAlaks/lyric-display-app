import { app, BrowserWindow, BrowserView, Menu, shell, dialog, ipcMain, nativeTheme } from 'electron';
...
function createWindow(route = '/') {
  ...
  // Ensure links opened via window.open/_blank use the system browser
  // so users get full navigation controls (back/forward, history, etc.)
...
// Open a simple in-app browser window with navigation controls
function openInAppBrowser(initialUrl) {
  try {
    ...
    inAppBrowserView = new BrowserView({
      webPreferences: {
        nodeIntegration: false,
        contextIsolation: true,
      }
    ...
    <body>
      ...
      <script>
        ...
        // Wire buttons to main process via preload API
        ...
        // Receive current location from main
...
ipcMain.handle('open-in-app-browser', (event, url) => {
  const initial = url || 'https://www.google.com';
  openInAppBrowser(initial);
});

// In-app browser navigation handlers
