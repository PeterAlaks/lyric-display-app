...
import { grantKaraokeMediaFile } from '../karaokeMediaProtocol.js';
...
export function registerFileHandlers({ getMainWindow }) {
  ...
  ipcMain.handle('karaoke:select-audio', async () => {
    try {
      const win = getMainWindow?.();
      const result = await dialog.showOpenDialog(win || undefined, {
        properties: ['openFile'],
      ...
      return {
        ...
        sourceUrl: grantKaraokeMediaFile(filePath, AUDIO_MIME_TYPES[extension] || 'audio/*'),
      };
    } catch (error) {
      console.error('Error selecting karaoke audio:', error);
