...
const getExportFrameUrl = () => (
  isDev
    ? 'http://localhost:5173/karaoke-export-frame'
    : 'http://127.0.0.1:4000#/karaoke-export-frame'
);
...
const sanitizeExportPayload = (payload = {}) => {
  ...
  return {
    ...
    title: String(payload.title || 'Karaoke'),
...
export function registerKaraokeExportHandlers() {
  ipcMain.handle('karaoke:cancel-export', () => {
    if (!activeExport) {
      return { success: true };
    }
  ...
  ipcMain.handle('karaoke:export-video', async (event, payload = {}) => {
    if (activeExport) {
      return { success: false, error: 'A karaoke export is already running' };
    }
    ...
    const saveResult = await dialog.showSaveDialog(win || undefined, {
      ...
      defaultPath: `${normalized.title || 'karaoke-video'}.mp4`,
    ...
    const sendProgress = (progress) => {
      try {
        event.sender.send('karaoke:export-progress', progress);
      } catch { }
    };
    ...
    try {
      ...
      await exportWindow.webContents.executeJavaScript(
        `window.__karaokeExportLoad(${JSON.stringify(normalized)})`,
        true
      );
      ...
      for (let frame = 0; frame < frameCount; frame += 1) {
        ...
        await exportWindow.webContents.executeJavaScript(
          `window.__karaokeExportSeek(${timelineTimeMs})`,
          true
        );
      ...
      return { success: false, error: error?.message || 'Failed to export karaoke video' };
    } finally {
      try {
